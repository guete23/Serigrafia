...
Public Class Pedidos_c
    ...
    Dim l As Integer = 1
    Dim op As String = " SERI11.STATUS|DESCRIPCION|NOMBRE_PROV|SERI11.PEDIDO|SERI11.FECHA"
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Pedidos_c))
        ...
        Me.criterio.Items.AddRange(New Object() {"ESTADO", "MATERIAL", "PROVEEDOR", "PO", "FECHA PO"})
...
#End Region
    ...
    Private Sub llena_grid()
        ...
        For Each dr In dt.Rows
            ...
            fg(l, 2) = dr("FECHA")
            fg(l, 3) = dr("F_ENTREGA")
            fg(l, 4) = dr("NOMBRE_PROV")
            fg(l, 5) = dr("CODIGO")
            fg(l, 6) = dr("DESCRIPCION")
            fg(l, 7) = dr("MONEDA")
            fg(l, 8) = dr("COSTO")
            fg(l, 9) = dr("UNIDADES")
            fg(l, 10) = dr("RECIBIDAS")
    ...
    Private Sub subtotales()
        Try
            ...
            fg.Subtotal(AggregateEnum.Sum, -1, -1, -1, 10, "Gran Total -->")
        Catch
        End Try
    End Sub
    ...
    Private Sub criterio_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles criterio.SelectedIndexChanged
        ...
        Select Case criterio.SelectedIndex
            Case 0
                strsql = "SELECT DISTINCT STATUS FROM SERI11 ORDER BY STATUS DESC"
            Case 1
                strsql = "SELECT DISTINCT DESCRIPCION FROM SERI11 LEFT JOIN SERI01 ON SERI11.CODIGO = SERI01.CODIGO ORDER BY DESCRIPCION DESC"
            Case 2
                strsql = "SELECT DISTINCT NOMBRE_PROV FROM SERI11 LEFT JOIN SERI10 ON SERI11.PEDIDO = SERI10.PEDIDO LEFT JOIN SERI04 ON SERI10.PROVEEDOR = SERI04.PROVEEDOR ORDER BY NOMBRE_PROV"
            Case 3
                strsql = "SELECT DISTINCT PEDIDO FROM SERI11 ORDER BY PEDIDO DESC "
            Case 4
                strsql = "SELECT DISTINCT CONVERT(VARCHAR(7), FECHA, 120) AS FECHA FROM SERI11 LEFT JOIN SERI10 ON SERI11.PEDIDO = SERI10.PEDIDO"
        End Select
    ...
    Private Sub procesa()
        ...
        '    busca = " SERI11.STATUS = 'A'"
        'End If
        'If criterio.SelectedIndex = 1 Then
        '    busca = "YEAR(FECHA) = '" & Mid(nombre.Text, 1, 4) & "' AND MONTH(FECHA) = '" & Mid(nombre.Text, 6) & "'"
        'End If
        If criterio.SelectedIndex = 4 Then
            busca = "YEAR(FECHA) = '" & Mid(nombre.Text, 1, 4) & "' AND MONTH(FECHA) = '" & Mid(nombre.Text, 6) & "'"
        End If
        strsql = "SELECT * FROM SERI11 LEFT JOIN SERI10 ON SERI11.PEDIDO = SERI10.PEDIDO LEFT JOIN SERI01 ON SERI11.CODIGO = SERI01.CODIGO LEFT JOIN SERI04 ON SERI10.PROVEEDOR = SERI04.PROVEEDOR WHERE " & busca
        llena_grid()
    End Sub
