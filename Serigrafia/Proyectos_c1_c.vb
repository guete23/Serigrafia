...
Public Class Proyectos_c1_c
    ...
    Dim l As Integer = 1
    Dim op As String = " SERI20.ESTADO|NOMBRE|FECHA_P|SERI20.PROYECTO"
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(Proyectos_c1_c))
        ...
        Me.criterio.Items.AddRange(New Object() {"ESTADO", "CLIENTE", "FECHA O.P.", "O.PROD."})
        ...
        Me.Name = "Proyectos_c1_c"
        Me.ShowInTaskbar = False
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Consulta de Proyectos"
...
#End Region
    ...
    Private Sub setea_grid()
        fg.Rows.Count = 1
        fg.Rows(0).Height = 35
        fg.Cols(12).DataType = GetType(Image)
    End Sub
    ...
    Private Sub llena_grid()
        ...
        For Each dr In dt.Rows
            ...
            fg(l, 8) = dr("PRIME")
            fg(l, 9) = dr("SEGUN")
            fg(l, 10) = fg(l, 7) - (fg(l, 8) + fg(l, 9))
            fg(l, 11) = dr("ESTADO")
            fg(l, 12) = _imgList.Images(0)
            l = l + 1
        Next dr
        subtotales()
    End Sub
    ...
    Private Sub criterio_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles criterio.SelectedIndexChanged
        ...
        Select Case criterio.SelectedIndex
            Case 0
                strsql = "SELECT DISTINCT ESTADO FROM SERI20 ORDER BY ESTADO "
            Case 1
                strsql = "SELECT DISTINCT NOMBRE FROM SERI20 LEFT JOIN SERI15 ON SERI20.NIT = SERI15.NIT ORDER BY NOMBRE"
            Case 2
                strsql = "SELECT DISTINCT CONVERT(VARCHAR(7), FECHA_P, 120) AS FECHA FROM SERI20 "
            Case 3
                strsql = "SELECT DISTINCT PROYECTO FROM SERI20 ORDER BY PROYECTO DESC"
        End Select
        llena_combos(nombre, strsql, "0")
    End Sub
    ...
    Private Sub procesa()
        Dim busca As String = o(criterio.SelectedIndex) & " =  '" & nombre.Text & "' ORDER BY SERI20.PROYECTO DESC"
        If criterio.SelectedIndex = 2 Then
            busca = "YEAR(FECHA_P) = '" & Mid(nombre.Text, 1, 4) & "' AND MONTH(FECHA_P) = '" & Mid(nombre.Text, 6) & "'"
        End If
        strsql = "SELECT * , (SELECT SUM(UNIDADES) FROM SERI23 WHERE SERI23.PROYECTO = SERI20.PROYECTO) AS RECIBIDAS, (SELECT SUM(PRIMERAS) FROM SERI24 WHERE SERI24.PROYECTO = SERI20.PROYECTO) AS PRIME , (SELECT SUM(SEGUNDAS) FROM SERI24 WHERE SERI24.PROYECTO = SERI20.PROYECTO) AS SEGUN FROM SERI20 LEFT JOIN SERI15 ON SERI20.NIT = SERI15.NIT WHERE " & busca
        llena_grid()
    End Sub
    ...
    Private Sub fg_Click(sender As System.Object, e As System.EventArgs) Handles fg.Click
        If fg.ColSel = 12 Then
            Dim forma As New Proyectos_c9
            forma.proyecto.Text = Format(fg(fg.RowSel, 1), "000000")
            forma.ShowDialog()
        End If
    End Sub
End Class
